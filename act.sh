...
#time act workflow_dispatch -W "${cwd}/.github/workflows/docker-image-prepare-amd64-arm64.yml" -s DH_USER=$(cat ~/DHUser.txt) -s DH_TOKEN=$(cat ~/DHToken.txt) -s GH_TOKEN=$(cat ~/GHToken.txt) -j docker-build-tag-push -a "${USER}" --container-options "-v /dev/:/dev" -n
...
#time act workflow_dispatch -W "${cwd}/.github/workflows/docker-image-scan-amd64-arm64.yml" -s DH_USER=$(cat ~/DHUser.txt) -s DH_TOKEN=$(cat ~/DHToken.txt) -s GH_TOKEN=$(cat ~/GHToken.txt) -j scan-on-linux -a "${USER}" --container-options "-v /dev/:/dev" -n
